...
exports.costWorking = costWorking;
exports.sumParts = sumParts;
exports.reconcile = reconcile;
exports.costExact = costExact;
exports.rate = rate;
exports.rateWindow = rateWindow;
...
function sumParts(working, keys) {
 ...
 */
function reconcile(working, storedCostUsd) {
    const stored = n(storedCostUsd);
    if (working === null || stored === null)
        return null;
...
function rateWindow(from, to) {
    ...
    if (bounded && end)
        return `in force ${day(start)} to ${day(end)}`;
    if (bounded)
        return `in force from ${day(start)}`;
    if (end)
        return `in force until ${day(end)}`;
