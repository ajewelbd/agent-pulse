...
/** The columns every caller already selects. snake_case: this is the query boundary. */
export interface TokenInputsRow {
  token_source: string;
...
export type TokenPartKey = 'input' | 'cache_read' | 'cache_write';

export interface TokenPart {
  key: TokenPartKey;
  label: string;
  tokens: number;
  /** What this bucket is, in one clause — these are not self-explanatory. */
  what: string;
}

export interface TokenWorking {
  parts: TokenPart[];
  totalIn: number;
  /** NULL only if the provider reported usage but omitted an output count. */
  output: number | null;
  /** The cache-write total broken down, when the provider split it. */
  writeSplit: {
    m5: number;
    h1: number;
    sum: number;
    /** The buckets add up to more than the cache-write total the same provider reported. */
 ...
 */
export function tokenWorking(row: TokenInputsRow | null | undefined): TokenWorking | null {
  if (!row || row.token_source === 'unknown') return null;
...
export function sumTokenParts(working: TokenWorking, keys: TokenPartKey[]): number {
...
export function shareOf(working: TokenWorking, part: TokenPart): number {
 ...
 * had reported zero.
 */
export function sumTokenWorking(rows: TokenInputsRow[]): {
  parts: TokenPart[];
  totalIn: number;
  output: number;
  reported: number;
  ...
  const totals = new Map<TokenPartKey, number>();
  let output = 0;
  let reported = 0;
  let unreported = 0;
  let template: TokenPart[] = [];
